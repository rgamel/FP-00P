...
function words(str) {
  // returns words from a string in an array
  return String(str)
    .toLowerCase()
    .split(/\s|\b/)
...
let wordsUsed1 = unique(wordsFound);

console.log(wordsUsed1); // should be a list of the unique words in the text

// // let's connect them
let wordsUsed2 = unique(words(text)); // see how it almost reads like a sentence?
...
let wordsUsed3 = uniqueWords(text); // the 3 values of wordsUsed are the same

console.log(wordsUsed3);
...
let uniqueWords2 = compose2(unique, words); // first class functions! aka functions can be treated like any other value

let wordsUsed4 = uniqueWords2(text);
