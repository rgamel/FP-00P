...

{
  ...
  // let's get our favorite numbers from an array.
  // faves are defined as being greater than or equal to the cutoff
...
} // probably [3, 4]? unless config.cutoff isn't what we expected

{
  // without side causes now
  const getFavesPure = (values, cutoff) => values.filter((v) => v >= cutoff);

  console.log(getFavesPure(nums, config.cutoff)); // [3, 4]
}
